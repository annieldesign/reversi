...
socket.on('join_room', function(payload){
	...
	/* When a web page disconnect alert everyone in the same rpm */
...
/* payload:
{
	...
	'column': 0-7 the column to play the token
...
socket.on('play_token', function(payload){
	...
	//Check that the player has previously regisitered
    ...
    if (('undefined' === typeof row) || row < 0 || row > 7) {
	  var error_message = 'play_token can\'t specify a vallid row, command aborted';
      log(error_message);
      socket.emit('play_token_response', {
        result: 'fail',
        message: error_message
    ...
    if (('undefined' === typeof column) || column < 0 || column > 7) {
	  var error_message = 'play_token can\'t specify a vallid column, command aborted';
      log(error_message);
      socket.emit('play_token_response', {
        result: 'fail',
        message: error_message
	...
	if  
		...
		var error_message = 'play_token turn play by wrong player';
		log(error_message);
		socket.emit('play_token_response', {
											result: 'fail',
											message: error_message
...
/* Check if the position at r, c contains the opposite od 'who' on the board 
and if the line indicated by adding dr to r and dc to c eventually end the color*/
...
function send_game_update(socket, game_id, message) {
	...
	/* Check to see if a game with game_id already exitst */
	...
	/* Make sure that only 2 people are in the same room */
	var roomObject;
    var numClients;
